...
| 6,2 | Trésor Caché | ✅ | ✅ | Mécanique dédiée (`Core.Rules.ExplorationOutcomeResolver.IsIndependentThresholdResult`/`MeetsIndependentThreshold`) : chaque objet de la liste a son propre jet D6 contre son propre seuil (`IndependentOutcomeEntry`, une ligne par `ExplorationOutcome`), plusieurs peuvent se déclencher à la fois - contrairement au reste du Groupe B (une seule branche résolue par archétype). Réutilise tels quels les blocs déjà construits (Entry+dé, `ChipView` tapable, table Artefact Magique) - mockup confirmé avant implémentation (2026-08-24) |
| 6,3 | Forge Naine | ✅ | ✅ | Règle Gromril attachée sur les 3 branches concernées |
| 6,4 | Bande Massacrée | ✅ | ✅ | Même mécanique que Trésor Caché (`IndependentOutcomeEntry`), aucun code en plus - couvre en plus une branche Auto de type Objet (Dagues, D6, pas de seuil) et plusieurs quantités variables (D3/D6, pas seulement "1" fixe) |
